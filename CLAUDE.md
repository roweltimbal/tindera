...
**Auth flow** — on login, hash-verify with Argon2, write a signed session to an HTTP-only `Secure` cookie. On each protected request, read and validate the cookie server-side in the route's Server Component or middleware. No client-side auth state. `src/lib/auth/auth-service.ts`'s `signUpUser` is fully implemented (Argon2 hash, store+user insert with orphaned-store rollback — see cross-collection create pattern below). `validateAndSignIn` (sign-in) and `src/lib/auth/session.ts` (cookie helpers) are still stubs — JWT/session-cookie logic is still TODO.
